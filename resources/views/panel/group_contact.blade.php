...
@section('navbarContent')
	...
	<div class="p-5 mt-4 bg-gray-50 rounded-lg border shadow">
		...
		<!-- Modal toggle -->
		<button class="btn-add-group_contact mb-4 block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
		...
		<div class="relative p-4 w-full max-w-md max-h-full">
			...
			<div class="relative bg-white rounded-lg shadow">
				...
				<div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
					<h3 id="group-contact-modal-title" class="text-lg font-semibold text-gray-900">
						Tambah Group
					</h3>
					<button type="button" class="btn-close-modal-group_contact text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
				...
				<form id="form_input" action="{{ route('group_contact.insert') }}" class="p-4 md:p-5" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="nama_group" class="block mb-2 text-sm font-medium text-gray-900">Nama Group</label>
                        <input type="text" name="nama_group" id="nama_group" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Type text" required="">
                    </div>
	...
	<div id="contact-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
		<div class="relative p-4 w-full max-w-md max-h-full">
			...
			<div class="relative bg-white rounded-lg shadow">
				...
				<!-- Modal body -->
				<form id="form_input_contact" action="{{ route('contact.insert') }}" class="p-4 md:p-5" method="POST">
					<input type="hidden" id="id_group_contact" name="id_group_contact" value="">
					@csrf
					<div class="mb-4">
	...
	<script>
		$(() => {
					...
					},
					{
						...
						render : (e,display,data) => {
							...
							return `
								...
								<div id="menu_dropdown_group_contact_${id}" class="menu-dropdown z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
									<ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
                                        <li>
											<a href="${url_detail}" class="block px-4 py-2 text-gray-500 hover:bg-gray-100">
												<i class="fas fa-fw fa-address-book"></i>
												List Contact
											</a>
										</li>
                                        <li>
											<a href="#" data-id="${id}" class="btn-edit-group_contact block px-4 py-2 text-gray-500 hover:bg-gray-100">
												<i class="fas fa-fw fa-pen-alt"></i>
												Edit
											</a>
										</li>
										<li>
											<a href="#" data-id="${id}" class="btn-delete-group_contact block px-4 py-2 text-red-500 hover:bg-gray-100">
			...
			$("body").on("click",".btn-add-contact",function(e) {
                ...
                contactModal.show();
                $("#form_input_contact")[0].reset();
            });

			$("body").on("submit","#form_input_contact",function(e) {
				e.preventDefault();
